...
define(["jquery", "underscore", "handlebars", "bootstrap", "bootstrap-dialog", "org/forgerock/openidm/ui/common/dashboard/widgets/AbstractWidget", "org/forgerock/commons/ui/common/main/ValidatorsManager", "org/forgerock/openidm/ui/admin/delegates/AuditDelegate", "org/forgerock/openidm/ui/common/util/QueryFilterEditor", "org/forgerock/openidm/ui/admin/util/SpinnerUtil", "org/forgerock/openidm/ui/admin/dashboard/widgets/util/TimezoneUtil", "selectize", "moment", "calHeatmap"], function ($, _, Handlebars, Bootstrap, BootstrapDialog, AbstractWidget, ValidatorsManager, AuditDelegate, QueryFilterEditor, SpinnerUtil, TimezoneUtil, Selectize, moment, CalHeatMap) {
        ...
        customSettingsLoad: function customSettingsLoad(dialogRef) {
            ...
            dialogRef.$modalBody.find(".hex, .color").bind("change", function (e) {
                ...
                if (changeInput.hasClass("hex")) {
                    ...
                    //test if real color
                    var isHexColor = /^#[0-9A-F]{6}$/i.test(curVal);
            ...
            this.model.cal.init(_.extend({
                ...
                onClick: function onClick(date, nb) {
                    ...
                    if (scope === "week") {
                        ...
                        dateRange = "timestamp+gt+\"" + queryDate.toISOString() + "\"+and+timestamp+lt+\"" + queryDate.endOf("hour").toISOString() + "\"";

                        AuditDelegate.getAuditData(_this.getAuditEvent(), dateRange).then(function (data) {
                            var prettyData = _.map(data.result, function (result) {
                            ...
                            BootstrapDialog.show({
                                ...
                                message: $(Handlebars.compile("{{> dashboard/widget/_NodeDetails}}")({ "entries": prettyData })),
                                buttons: [{
