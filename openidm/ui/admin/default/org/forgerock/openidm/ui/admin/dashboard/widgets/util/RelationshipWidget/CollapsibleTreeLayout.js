...
define(["jquery", "underscore", "d3", "org/forgerock/openidm/ui/common/delegates/ResourceDelegate"], function ($, _, d3, ResourceDelegate) {
        ...
        _.each(relationshipProps, function (prop) {
            ...
            if (propDetails) {
                if (propDetails.length || prop.type === "relationship" && !_.isEmpty(propDetails)) {
                    ...
                    /*
                     * if any of the values in the array of relationships come from repo/internal
    ...
    */
    ...
    obj.generateChartDisplay = function () {
        ...
        // define the baseSvg
        baseSvg = d3.select(".relationshipGraph").append("svg").attr("width", svgWidth).attr("height", svgHeight).attr("class", "overlay"),
        ...
        updateChart = function updateChart(sourceNode) {
            ...
            nodeEnter = chartNode.enter().append("g").attr("class", "node").attr("transform", function (d) {
            ...
            nodeExit = chartNode.exit().transition().duration(duration).attr("transform", function (d) {
