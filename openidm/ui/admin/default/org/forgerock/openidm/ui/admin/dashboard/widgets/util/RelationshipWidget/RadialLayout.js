...
define(["jquery", "underscore", "d3"], function ($, _, d3) {
    ...
    obj.setupChart = function (widget, details) {
            ...
            if (details[prop.propName]) {
                if (details[prop.propName].length || prop.type === "relationship" && !_.isEmpty(details[prop.propName])) {
                    ...
                    if (!(prop.type === "array" && _.reject(details[prop.propName], function (item) {
                        return item._ref.indexOf("repo/internal") === 0;
                    }).length === 0)) {
        ...
        */
        ...
        obj.updateRoundChart(focalNodeID, nodeSet, linkSet, ".relationshipGraph", "colorScale20c");
    };
    ...
    obj.nodeMouseOut = function () {
        ...
        d3.select(this).select("circle").transition().duration(250).attr("r", function (d, i) {
            if (d.id === focalNode) {
    ...
    obj.updateRoundChart = function (focalNode, nodeSet, linkSet, selectString) {
        ...
        //Create a canvas
        svgCanvas = d3.select(selectString).append("svg:svg").attr("width", width).attr("height", height).append("svg:g").attr("class", "focalNodeCanvas").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
