...
define(["jquery", "underscore", "moment"], function ($, _, moment) {
    ...
    /**
     * Given a dialog reference with a .timezone-group this will set the group of inputs
     * to reflect the offset of the browser.
     *
     * @param dialogRef
     */
    obj.setLocalOffset = function (dialogRef) {
        if (dialogRef.$modalBody.find(".timezone-group").length > 0) {

            var offset = this.getLocalOffset();
            dialogRef.$modalBody.find(".offset-minutes").val(offset.minutes);
            dialogRef.$modalBody.find(".offset-hours").val(offset.hours);

            if (offset.negative) {
                dialogRef.$modalBody.find(".plus-minus").val("-");
            } else {
                dialogRef.$modalBody.find(".plus-minus").val("+");
            }
        }
    ...
    /**
     * Given a dialog reference with a .timezone-group this will retrieve an object representing
     ...
     * @returns {{negative: boolean, hours: string, minutes: string}}
     */
    obj.getTimezoneOffset = function (dialogRef) {
        if (dialogRef.$modalBody.find(".timezone-group").length > 0) {
            var minutes = parseInt(dialogRef.$modalBody.find(".offset-minutes").val(), 10),
                hours = parseInt(dialogRef.$modalBody.find(".offset-hours").val(), 10),
                negative = dialogRef.$modalBody.find(".plus-minus").val() === "-";
