...
define(["jquery", "underscore", "org/forgerock/commons/ui/common/main/AbstractView", "org/forgerock/commons/ui/common/main/Configuration", "org/forgerock/commons/ui/common/main/EventManager", "org/forgerock/commons/ui/common/util/Constants", "org/forgerock/commons/ui/common/util/UIUtils", "bootstrap-dialog", "org/forgerock/openidm/ui/common/util/ResourceCollectionUtils"], function ($, _, AbstractView, conf, eventManager, constants, uiUtils, BootstrapDialog, ResourceCollectionUtils) {
    var ResourceCollectionSearchDialog = AbstractView.extend({
        ...
        render: function render(opts, onLoadCallback) {
            ...
            //get the all the managed object and connector schemas
            ResourceCollectionUtils.getAllAvailableResourceCollections().then(function (allResourceCollections) {
                //it this is a new relationship remove system objects from the list of resources available
                if (isNew) {
                ...
                //change dialog
                _this2.dialog = BootstrapDialog.show({
                    title: title,
        ...
        loadTemplate: function loadTemplate() {
            uiUtils.renderTemplate(this.template, this.currentDialog, _.extend({}, conf.globalData, this.data), _.bind(function () {
        ...
        setupResourceTypeField: function setupResourceTypeField(typePath) {
            var _this = this,
                ...
                onChange: _.bind(function (val) {
                    this.setupResourceField(ResourceCollectionUtils.getResourceCollectionIndex(this.schema, val, this.data.property.propName, this.model.allAvailableResourceCollections));
                }, this)
            };
            ...
            if (this.data.property.items) {
                if (!this.data.property.items.resourceCollection.length) {
                    this.data.property.items.resourceCollection = this.model.allAvailableResourceCollections;
                }
            } else {
                if (!this.data.property.resourceCollection.length) {
                    this.data.property.resourceCollection = this.model.allAvailableResourceCollections;
                }
            }
        ...
        setupResourceField: function setupResourceField(resourceCollectionIndex) {
            var _this3 = this;
            ...
            if (resourceCollection && resourceCollection.path.split("/")[0] === "system" || resourceCollection && resourceCollection.path.split("/")[0] === "managed") {
            ...
            if (resourceTypeValue && resourceTypeValue.length || resourceCollectionIndex === 0) {
                ResourceCollectionUtils.setupAutocompleteField(autocompleteField, this.data.property, { maxItems: this.maxSelectionItems }, resourceCollectionIndex, this.data.propertyValue).then(function () {
                    _this3.currentDialog.find("#relationshipForm").show();
                });
            } else {
        ...
        */
        getNewValArray: function getNewValArray(refPropsOverride) {
            var propVal = this.data.propertyValue,
                ...
                getRefProps = function getRefProps() {
            ...
            if (!_.isArray(relationshipValue)) {
                ...
                _.each(relationshipValue, function (val) {
