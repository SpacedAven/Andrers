...
define(["jquery", "lodash", "org/forgerock/commons/ui/common/main/AbstractView", "org/forgerock/commons/ui/common/main/ValidatorsManager", "org/forgerock/commons/ui/common/main/Configuration"], function ($, _, AbstractView, ValidatorsManager, Configuration) {
    var PersonalInfoItemView = AbstractView.extend({
         ...
         */
        cancelChanges: function cancelChanges(event) {
            this.$el.find("#" + this.model.schemaName + "Collapse").collapse("hide");
        },
        ...
        saveChanges: function saveChanges(event) {
            event.preventDefault();
            if (ValidatorsManager.formValidated(this.$el.find("form")) && !_.isEqual(this.model.value, this.data.value)) {
                this.submit(_defineProperty({}, this.model.schemaName, this.data.value));
            } else {
                this.cancelChanges(event);
            }
        },
         ...
         */
        collapseHideHandler: function collapseHideHandler(event) {
            this.resetValue();
         ...
         */
        collapseShowHandler: function collapseShowHandler(event) {
         ...
         */
        collapseShownHandler: function collapseShownHandler(event) {
